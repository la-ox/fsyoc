...
class Blockquote extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::TEXT,
				'label_block' => false,
				'placeholder' => __( 'via @username', 'elementor-pro' ),
				'condition' => [
					'tweet_button' => 'yes',
				],
			]
	...
	protected function render() {
		...
		$share_link = 'https://twitter.com/intent/tweet?text=' . $settings['blockquote_content'];
		$via = !empty( $settings['user_name'] ) ? ' ' . $settings['user_name'] : '';

		if ( ! empty( $settings['author_name'] ) ) {
			$share_link .= " -" . $settings['author_name'];
		}
			...
			<?php if ( ! empty( $settings['author_name'] ) || 'yes' === $settings['tweet_button'] ) : ?>
				<footer>
					...
					<?php if ( 'yes' === $settings['tweet_button'] ) : ?>
						<a href="<?php echo $share_link . $via; ?>" class="elementor-blockquote__tweet-button" target="_blank">
